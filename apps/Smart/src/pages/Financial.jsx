...
Responde con: 1) resumen de 2 oraciones, 2) un punto positivo, 3) una recomendación. Máximo 100 palabras. Usa emojis con moderación.`;
  ...
  return (
    <div className="min-h-screen bg-black/95 flex flex-col">
      ...
      {/* ── Modal desglose Neto ────────────────────────────────────────────── */}
      <Dialog open={showNetoBreakdown} onOpenChange={setShowNetoBreakdown}>
        <DialogContent className="max-w-sm bg-[#0A0A0A] border border-cyan-500/20 rounded-2xl p-0 overflow-hidden">
          <div className="absolute top-0 left-0 w-full h-0.5 bg-gradient-to-r from-cyan-500 to-emerald-500" />
          <div className="p-5 space-y-4">
            <DialogHeader>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <DialogTitle className="text-lg font-black text-white tracking-tight text-left">Desglose de Ganancia</DialogTitle>
                  <p className="text-[11px] text-white/30 font-medium">Cómo se calculó tu neto real</p>
                </div>
              </div>
            </DialogHeader>
            ...
            <div className="space-y-1">
              ...
              <div className="flex items-center justify-between py-2.5 border-b border-white/[0.06]">
                <div>
                  <p className="text-sm font-bold text-white">Ingresos brutos</p>
                  <p className="text-[10px] text-white/30">Total cobrado a clientes</p>
                </div>
                <p className="text-sm font-black text-emerald-400">+${totalRevenue.toFixed(2)}</p>
              </div>

              {/* IVU */}
              <div className="flex items-center justify-between py-2.5 border-b border-white/[0.06]">
                <div>
                  <p className="text-sm font-bold text-white/70">IVU / Impuesto cobrado</p>
                  <p className="text-[10px] text-white/30">No es dinero tuyo — se reporta al gobierno</p>
                </div>
                <p className="text-sm font-black text-red-400">−${totalIVU.toFixed(2)}</p>
              </div>

              {/* Ingresos sin IVU */}
              <div className="flex items-center justify-between py-2.5 border-b border-white/[0.06] bg-white/[0.02] rounded-xl px-2">
                <div>
                  <p className="text-sm font-black text-white">Ingresos sin IVU</p>
                  <p className="text-[10px] text-white/30">Lo que realmente recibiste</p>
                </div>
                <p className="text-sm font-black text-white">${totalRevenueBeforeTax.toFixed(2)}</p>
              </div>

              {/* Costo piezas/servicios */}
              <div className="flex items-center justify-between py-2.5 border-b border-white/[0.06]">
                <div>
                  <p className="text-sm font-bold text-white/70">Costo de piezas / servicios</p>
                  <p className="text-[10px] text-white/30">Lo que costaron los productos vendidos</p>
                </div>
                <p className="text-sm font-black text-orange-400">−${totalPartsCost.toFixed(2)}</p>
              </div>

              {/* Margen bruto */}
              <div className="flex items-center justify-between py-2.5 border-b border-white/[0.06] bg-white/[0.02] rounded-xl px-2">
                <div>
                  <p className="text-sm font-black text-white">Margen bruto</p>
                  <p className="text-[10px] text-white/30">Sin IVU ni costo de piezas</p>
                </div>
                <p className={`text-sm font-black ${grossMargin >= 0 ? "text-cyan-400" : "text-red-400"}`}>${grossMargin.toFixed(2)}</p>
              </div>

              {/* Gastos operacionales */}
              <div className="flex items-center justify-between py-2.5 border-b border-white/[0.06]">
                <div>
                  <p className="text-sm font-bold text-white/70">Gastos operacionales</p>
                  <p className="text-[10px] text-white/30">Renta, utilidades, nómina, etc.</p>
                </div>
                <p className="text-sm font-black text-red-400">−${totalExpenses.toFixed(2)}</p>
              </div>

              {/* Ganancia neta real */}
              <div className={`flex items-center justify-between py-3 mt-2 rounded-2xl px-3 border ${trueNetProfit >= 0 ? "bg-emerald-500/10 border-emerald-500/20" : "bg-red-500/10 border-red-500/20"}`}>
                <div>
                  <p className="text-base font-black text-white">Ganancia neta real</p>
                  <p className="text-[10px] text-white/40">Después de todo</p>
                </div>
                <p className={`text-lg font-black ${trueNetProfit >= 0 ? "text-emerald-400" : "text-red-400"}`}>
                  {trueNetProfit < 0 ? "−" : "+"}${Math.abs(trueNetProfit).toFixed(2)}
                </p>
              </div>

              {/* Diferencia con el Neto básico si hay costo de piezas */}
              {totalPartsCost > 0 && (
                <div className="bg-amber-500/[0.07] border border-amber-500/20 rounded-2xl px-3 py-2.5 mt-1">
                  <p className="text-[11px] text-amber-300/80 leading-relaxed">
                    💡 El "Neto" que ves en el panel ({netProfit >= 0 ? "+" : "−"}${Math.abs(netProfit).toFixed(2)}) no descuenta el costo de piezas. Tu ganancia real es <strong>${trueNetProfit.toFixed(2)}</strong>.
                  </p>
                </div>
              )}
            </div>
            ...
            <button
              onClick={() => setShowNetoBreakdown(false)}
              className="w-full h-11 rounded-2xl bg-white/5 border border-white/10 text-white/60 font-black text-sm hover:bg-white/10 transition-all active:scale-95"
            >
              Cerrar
            </button>
          </div>
