    ...
    >
      ...
      >
        <svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg" style={{ width: "100%", height: "100%", animation: "sfx-gear-spin-mini 14s linear infinite", transformOrigin: "50% 50%" }}>
          ...
          <g clipPath="url(#login-gear-clip)">
...
export default function Login() {
  ...
  const [stage, setStage] = useState("form"); // 'form' | 'sending' | 'sent' | 'google'
  ...
  const submitMagicLink = async (e) => {
    ...
    setStage("sending");
    try {
      await signInWithMagicLink({ email: clean });
      setStage("sent");
    } catch (err) {
  ...
  const startGoogle = async () => {
    ...
    try {
      ...
      setError(err.message || "Google rechazó la conexión.");
      setStage("form");
    }
  };
          ...
          <>
            ...
            >
              ...
              <p className="mt-3 text-[15px] text-white/55 leading-relaxed">
                Entra a tu dashboard con el mismo email que usaste al registrarte.
              </p>
            </motion.div>
            ...
            >
              ...
              <button
                type="submit"
                disabled={stage === "sending"}
                className="mt-2 w-full h-12 rounded-full bg-lime-400 text-black font-semibold text-sm inline-flex items-center justify-center gap-2 hover:bg-lime-300 transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
              >
                {stage === "sending" ? (
                  <>
                    <Loader2 className="h-4 w-4 animate-spin" />
