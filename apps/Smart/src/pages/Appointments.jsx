  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.06] shrink-0">
        <button onClick={() => navigate(-1)} className="w-9 h-9 rounded-xl bg-white/[0.05] flex items-center justify-center">
          <ChevronLeft className="w-4 h-4 text-white/50" />
        </button>
        <div className="text-center">
          <h1 className="text-base font-black text-white">Calendario de Citas</h1>
        ...
        <button
          ...
          className="w-9 h-9 rounded-xl bg-violet-600 hover:bg-violet-500 flex items-center justify-center transition-colors"
        >
      ...
      {/* Cabeceras de días */}
      <div className="grid grid-cols-8 border-b border-white/[0.06] shrink-0">
        ...
        {days.map((d, i) => {
          ...
          return (
            <div key={i} className="py-2 text-center">
              ...
              <p className={`text-sm font-black leading-none mt-0.5 ${isToday ? "text-violet-400" : "text-white/70"}`}>
                {d.getDate()}
              </p>
              {apptsForDay(d).length > 0 && (
                <div className="flex justify-center mt-0.5 gap-0.5">
                  {apptsForDay(d).slice(0, 3).map((_, j) => (
                    <div key={j} className="w-1 h-1 rounded-full bg-violet-500" />
                  ))}
                </div>
              )}
            </div>
            ...
            {days.map((day, di) => {
              ...
              return (
                <div
                  ...
                  className="border-l border-white/[0.04] relative cursor-pointer hover:bg-white/[0.02] transition-colors"
                >
                  {appt && (
                    <div className="absolute inset-x-0.5 top-0.5 rounded-lg bg-violet-600/80 border border-violet-500/50 px-1.5 py-1 overflow-hidden">
                      <p className="text-[9px] font-black text-white leading-tight truncate">{appt.customer_name}</p>
                      <p className="text-[8px] text-violet-200/70 leading-tight truncate">{appt.device_brand} {appt.device_model}</p>
                    </div>
                  )}
                </div>
              );
      ...
      {viewAppt && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/70 backdrop-blur-sm p-4"
          ...
          <div className="w-full max-w-sm rounded-3xl bg-[#0e0e0e] border border-white/10 p-5 space-y-4"
            onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <h3 className="text-base font-black text-white">Detalle de Cita</h3>
            ...
            <button
              ...
              className="w-full py-2.5 rounded-2xl bg-violet-600 hover:bg-violet-500 text-sm font-bold text-white transition-colors">
              Ver orden completa
            </button>
          </div>
        </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/70 backdrop-blur-sm p-4"
      ...
      <div className="w-full max-w-sm rounded-3xl bg-[#0e0e0e] border border-white/10 p-5 space-y-4"
        onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between">
          <h3 className="text-base font-black text-white">Nueva Cita</h3>
          ...
          {[
            ...
            <div key={key} className="space-y-1">
              ...
              <input
                ...
                className="w-full px-3 py-2.5 rounded-xl bg-white/[0.06] border border-white/[0.08] text-white text-sm focus:outline-none focus:border-violet-500/50"
              />
            </div>
          ))}
          <div className="space-y-1">
            <label className="text-[10px] font-bold text-white/30 uppercase tracking-widest">Motivo / Problema</label>
            <textarea
              rows={2}
              ...
              className="w-full px-3 py-2.5 rounded-xl bg-white/[0.06] border border-white/[0.08] text-white text-sm focus:outline-none focus:border-violet-500/50 resize-none"
            />
          </div>
        </div>
        ...
        <button
          onClick={save}
          disabled={saving}
          className="w-full py-3 rounded-2xl bg-violet-600 hover:bg-violet-500 disabled:opacity-50 text-sm font-black text-white transition-colors"
        >
