...
export function usePushNotifications() {
  ...
  const notify = useCallback(async ({ title, body, tag, url }) => {
    ...
    if (sw) {
      ...
      new window.Notification(title, { body, tag, icon: "/icon-192.png" });
    }
  }, []);

  return { notify, requestPermission, permission: typeof window !== "undefined" ? window.Notification?.permission ?? "default" : "default" };
}
