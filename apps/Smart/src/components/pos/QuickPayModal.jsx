...
export default function QuickPayModal({ order, paymentMode = "full", onClose, onSuccess }) {
  ...
  if (successData) {
    ...
    return (
      <div className="apple-type fixed inset-0 z-[99999] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
        <div className="apple-surface-elevated rounded-apple-lg shadow-apple-xl border-0 p-8 w-full max-w-sm text-center space-y-6">
          {/* Check icon */}
          <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-apple-green/15">
            <svg className="w-10 h-10 text-apple-green" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
            </svg>
          </div>

          <div>
            <p className="apple-text-caption1 font-semibold apple-label-tertiary">Pago completado</p>
            <p className="apple-text-title1 apple-label-primary mt-1 tabular-nums">${Number(successData.amountPaid).toFixed(2)}</p>
            {order?.customer_name && (
              <p className="apple-text-subheadline apple-label-secondary mt-1">{order.customer_name}</p>
            )}
            <p className="apple-text-caption1 apple-label-tertiary mt-1">{successData.saleNumber}</p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {waHref ? (
              <a href={waHref} target="_blank" rel="noreferrer"
                className="apple-press flex flex-col items-center gap-1.5 rounded-apple-md bg-apple-green/12 px-3 py-3 text-apple-green transition">
                ...
                <span className="apple-text-caption2 font-semibold">WhatsApp</span>
              </a>
            ) : (
              <div className="flex flex-col items-center gap-1.5 rounded-apple-md bg-gray-sys6 dark:bg-gray-sys5 px-3 py-3 apple-label-tertiary">
            ...
            {mailHref ? (
              <a href={mailHref}
                className="apple-press flex flex-col items-center gap-1.5 rounded-apple-md bg-apple-blue/12 px-3 py-3 text-apple-blue transition">
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                </svg>
                <span className="apple-text-caption2 font-semibold">Email</span>
              </a>
            ) : (
              <div className="flex flex-col items-center gap-1.5 rounded-apple-md bg-gray-sys6 dark:bg-gray-sys5 px-3 py-3 apple-label-tertiary">
            ...
            <button onClick={() => window.print()}
              className="apple-press flex flex-col items-center gap-1.5 rounded-apple-md bg-apple-purple/12 px-3 py-3 text-apple-purple transition">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" />
              </svg>
              <span className="apple-text-caption2 font-semibold">Imprimir</span>
            </button>
          </div>

          <button onClick={onClose}
            className="apple-btn apple-btn-secondary w-full h-12">
            Cerrar
          </button>
        </div>
      </div>
