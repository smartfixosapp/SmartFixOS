...
Gracias por su compra
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-md w-[95vw] sm:w-full max-h-[85vh] bg-gradient-to-br from-[#2B2B2B] to-black border-red-600/30 overflow-hidden flex flex-col p-0">
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-3 scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent">
          <div className="bg-white/5 border border-white/10 rounded-lg p-3 sm:p-4 space-y-2 sm:space-y-3 max-w-full overflow-x-hidden">
            <div className="text-center pb-2 sm:pb-3 border-b border-white/10">
              ...
              />
              <p className="text-[10px] sm:text-xs text-gray-400">Recibo de Venta</p>
            </div>
            ...
            <div className="border-t border-white/10 pt-2 sm:pt-3">
              <p className="text-[10px] sm:text-xs font-semibold text-gray-400 mb-1.5 sm:mb-2">ITEMS</p>
              <div className="space-y-1 sm:space-y-1.5">
                {saleData.items.map((item, idx) => (
                  <div key={idx} className="flex justify-between text-[10px] sm:text-xs">
                    <div className="flex-1 min-w-0 pr-2">
                      <p className="text-white truncate text-xs sm:text-sm">{item.name}</p>
                      <p className="text-gray-500 text-[9px] sm:text-[10px]">${item.price.toFixed(2)} × {item.quantity}</p>
                    </div>
                    <p className="text-white font-semibold text-xs sm:text-sm flex-shrink-0">${(item.price * item.quantity).toFixed(2)}</p>
                  </div>
                ))}
              </div>
            </div>

            <div className="border-t border-white/10 pt-2 sm:pt-3 space-y-1 sm:space-y-1.5">
              ...
              <div className="flex justify-between items-center pt-1.5 sm:pt-2 border-t border-white/10">
                ...
                <span className="text-xl sm:text-2xl font-bold text-emerald-400">${saleData.total.toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-[10px] sm:text-xs text-gray-400">
