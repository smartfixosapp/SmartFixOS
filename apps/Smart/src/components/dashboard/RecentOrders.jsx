...
export default function RecentOrders({ orders, loading }) {
  ...
  return (
    <Card className="bg-black/40 backdrop-blur-3xl border border-white/5 rounded-[24px] shadow-2xl relative overflow-hidden text-white">
          ...
          <div className="space-y-3">
            {orders.map((order) => (
              <Link key={order.id} to={createPageUrl(`Orders?order=${order.id}`)}>
                <div className="flex items-center justify-between p-4 border border-white/5 rounded-[20px] hover:bg-white/5 transition-all duration-200 cursor-pointer bg-white/5">
