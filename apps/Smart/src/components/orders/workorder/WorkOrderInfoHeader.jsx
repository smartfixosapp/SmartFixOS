...
import {
  ...
  CheckCircle2, XCircle, Image as ImageIcon, ChevronDown, ChevronUp, Mail, Printer
} from "lucide-react";
      ...
      <header className="flex-shrink-0 sticky top-0 z-30 border-b border-white/10 bg-[#0D0D0D]/80 backdrop-blur supports-[backdrop-filter]:backdrop-blur-md">
        <div className="px-3 sm:px-4 py-3 flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <Badge className={`text-[10px] rounded-full border px-2.5 py-0.5 ${pr.cls}`}>
              {pr.label}
            </Badge>
            
            <Badge variant="outline" className="text-[10px] rounded-full border-emerald-500/30 text-emerald-400 bg-emerald-500/5 px-2.5 py-0.5">
              {asMoney(balance)}
            </Badge>

            <Button onClick={handleGoToPOS} className="hidden sm:inline-flex bg-green-600 hover:bg-green-500 rounded-full h-8 text-xs font-semibold px-4 shadow-lg shadow-green-900/20">
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-2">
                    {order?.customer_phone && (
                      <>
                        <a 
                          href={`tel:${rawPhone}`} 
                          className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 hover:bg-white/10 border border-white/5 text-xs font-medium text-white transition-all active:scale-95"
                        >
                    ...
                    {order?.customer_email ? (
                      ...
                      <span className="text-xs text-gray-500 italic px-2">Sin email</span>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <div className="mt-1 flex items-center gap-2">
                    ...
                    <Button size="icon" variant="ghost" className="h-9 w-9" onClick={() => copyToClipboard(pin)} title="Copiar PIN" aria-label="Copiar PIN al portapapeles">
                ...
                <div>
                  ...
                  <div className="mt-1 flex items-center gap-2">
                    ...
                    <Button size="icon" variant="ghost" className="h-9 w-9" onClick={() => copyToClipboard(pass)} title="Copiar password" aria-label="Copiar contraseña al portapapeles">
                      <Copy className="w-4 h-4" />
                    </Button>
                  </div>
