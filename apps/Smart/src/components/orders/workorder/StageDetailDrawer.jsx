...
import PartArrivedStage from './stages/PartArrivedStage';
...
const stagesConfig = {
  ...
  part_arrived_waiting_device: { label: 'Pieza lista / Esperando cliente', icon: Check },
...
const stageIdToStatusMap = {
  ...
  part_arrived_waiting_device: 'part_arrived_waiting_device',
...
export default function StageDetailDrawer({ open, onClose, stageId, order, onUpdate, user }) {
  ...
  const renderStageContent = () => {
    ...
    switch (stageId) {
      ...
      case 'part_arrived_waiting_device':
        return <PartArrivedStage order={order} onUpdate={onUpdate} />;
