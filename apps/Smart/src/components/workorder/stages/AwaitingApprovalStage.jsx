...
export default function AwaitingApprovalStage({ order, onUpdate, compact }) {
  const o = order || {};
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap items-end gap-x-4 gap-y-2">
                <h2 className="text-3xl font-semibold tracking-tight text-white sm:text-4xl">Esperando Aprobación</h2>
            ...
            <div className="grid gap-3 md:grid-cols-3">
              ...
              <div className="rounded-[22px] border border-white/10 bg-black/25 p-4 backdrop-blur-md">
                <p className="mb-1 text-[11px] font-semibold tracking-[0.24em] text-white/35">Estimado</p>
                <p className="text-lg font-semibold text-white">${totals.total.toFixed(2)}</p>
              </div>
              <div className="rounded-[22px] border border-white/10 bg-black/25 p-4 backdrop-blur-md">
                <p className="mb-1 text-[11px] font-semibold tracking-[0.24em] text-white/35">Items</p>
              ...
              {o.customer_approval_status && (
                <div className={`rounded-[22px] border p-4 backdrop-blur-md ${
                  ...
                  <p className="mb-1 text-[11px] font-semibold tracking-[0.24em] text-white/35">Respuesta</p>
                  <p className={`text-lg font-semibold ${o.customer_approval_status === "approved" ? "text-emerald-300" : "text-red-300"}`}>
          ...
          <div className="grid gap-3">
            <div className="rounded-[26px] border border-yellow-400/15 bg-black/25 p-5 backdrop-blur-md">
              <div className="flex items-start gap-3 mb-4">
                ...
                <div>
                  <p className="text-[11px] font-semibold tracking-[0.28em] text-white/35">Acciones rápidas</p>
                  <h3 className="mt-0.5 text-base font-semibold text-white">Contactar al cliente</h3>
                </div>
              </div>
              <div className="space-y-2">
                {/* Share approval link */}
        ...
        <div className="space-y-3">
          {o.customer_approval_status && (
            <div className={`rounded-[22px] border p-4 ${
              ...
              <p className="mb-1 text-[11px] font-semibold tracking-[0.24em] text-white/35">Respuesta del cliente</p>
              <p className={`text-lg font-semibold ${o.customer_approval_status === "approved" ? "text-emerald-300" : "text-red-300"}`}>
                {o.customer_approval_status === "approved" ? "Aprobado" : "Rechazado"}
              </p>
            </div>
          ...
          {items.length === 0 ? (
            ...
            <div className="overflow-hidden rounded-[22px] border border-white/10 bg-black/25">
                  ...
                  return (
                    <div key={`${item?.id || item?.name}-${idx}`} className="flex items-center justify-between gap-4 p-4 hover:bg-white/[0.03] transition-colors">
                      ...
                      <div className="shrink-0 text-right">
                        ...
                        <p className="text-sm font-semibold text-yellow-300">${lineTotal.toFixed(2)}</p>
                      </div>
                    </div>
                  );
                })}
              ...
              <div className="border-t border-white/10 bg-black/35 px-5 py-3 flex justify-between items-center">
                ...
                <span className="text-lg font-semibold text-yellow-300">${totals.total.toFixed(2)}</span>
              </div>
            </div>
          )}
        </div>
