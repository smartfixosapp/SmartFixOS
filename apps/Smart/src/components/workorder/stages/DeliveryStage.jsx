...
export default function DeliveryStage({
  ...
  return (
    <div className="space-y-4">
      ...
      {/* ── COMPACT: financial summary + notify client ────────────────────── */}
      {compact && (
        <div className="space-y-3">
          <div className="rounded-[22px] border border-white/10 bg-black/25 p-4">
            <div className="flex items-center justify-between mb-2">
              <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-white/35">Resumen financiero</p>
              <Badge
                variant="outline"
                className={`rounded-full px-2 py-0.5 text-[10px] flex items-center gap-1 ${
                  isSaldado
                    ? "border-emerald-400/25 bg-emerald-500/10 text-emerald-200"
                    : "border-amber-400/25 bg-amber-500/10 text-amber-200"
                }`}
              >
                {isSaldado ? <CheckCircle2 className="w-2.5 h-2.5" /> : <AlertCircle className="w-2.5 h-2.5" />}
          ...
          {(digits || email) && (
            <div className="flex flex-wrap gap-2">
              {digits && (
                <a href={`tel:+${intl}`}
                  className="flex items-center gap-2 rounded-xl border border-white/15 bg-white/5 px-3 py-2 text-sm font-semibold text-white hover:bg-white/10 transition-all active:scale-95">
                  <PhoneCall className="w-3.5 h-3.5 text-white/60" />Llamar
                </a>
              )}
              {digits && (
                <a href={`https://wa.me/${intl}?text=${encodeURIComponent(`Hola ${o.customer_name || ""}, tu equipo ${o.device_brand || ""} ${o.device_model || ""} esta listo para recoger. Orden: ${o.order_number || ""}`)}`}
                  target="_blank" rel="noreferrer"
                  className="flex items-center gap-2 rounded-xl border border-emerald-500/25 bg-emerald-500/10 px-3 py-2 text-sm font-semibold text-emerald-300 hover:bg-emerald-500/20 transition-all active:scale-95">
                  <MessageCircle className="w-3.5 h-3.5" />Notificar
                </a>
              )}
              {email && (
                <a href={`mailto:${email}`}
                  className="flex items-center gap-2 rounded-xl border border-blue-500/25 bg-blue-500/10 px-3 py-2 text-sm font-semibold text-blue-300 hover:bg-blue-500/20 transition-all active:scale-95">
