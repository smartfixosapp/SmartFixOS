...
export default function TermsStep({ formData, updateFormData, config }) {
  ...
  return (
    <div className="apple-type space-y-6">
      <div>
        <h3 className="apple-text-title2 apple-label-primary mb-2">Términos y Condiciones</h3>
        <p className="apple-text-subheadline apple-label-secondary">Por favor lee y acepta los términos</p>
      </div>

      <Card className="apple-card p-6 max-h-96 overflow-y-auto">
        <div
          className="prose prose-sm max-w-none apple-label-primary apple-text-body"
          dangerouslySetInnerHTML={{ __html: termsText }}
        />
      </Card>

      <div className="flex items-start gap-3 p-4 apple-card apple-press">
        <Checkbox
          id="terms"
        ...
        <Label
          htmlFor="terms"
          className="apple-text-body apple-label-primary font-medium cursor-pointer"
        >
      ...
      {!formData.terms_accepted && (
        <div className="apple-text-footnote text-apple-orange flex items-center gap-2">
          <span>⚠️</span>
