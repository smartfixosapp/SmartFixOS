...
export default function WorkOrderWizard({
  ...
  const handleSubmit = async () => {
    setLoading(true);
    try {
      const fullCustomerName = `${formData.customer.name} ${formData.customer.last_name}`.trim();
      ...
      for (const mf of formData.media_files || []) {
        if (mf?.url && /^https?:\/\//i.test(String(mf.url))) {
        ...
        const sourceFile =
          mf instanceof File || mf instanceof Blob
            ? mf
            : (mf?.file instanceof File || mf?.file instanceof Blob)
            ? mf.file
            : null;
        if (!sourceFile) continue;

        const name =
          sourceFile instanceof File && sourceFile.name ? sourceFile.name : `photo-${Date.now()}.jpg`;
        const file =
          sourceFile instanceof File
            ? sourceFile
            : new File([sourceFile], name, { type: sourceFile.type || "image/jpeg" });
        const uploadResult = await base44.integrations.Core.UploadFile({ file });
        const fileUrl = pickUploadUrl(uploadResult);
        if (!fileUrl) throw new Error("Upload sin URL pública válida");
        photos_metadata.push({
          id: `ph-${Date.now()}-${name}`,
          ...
          filename: name,
          publicUrl: `${fileUrl}${fileUrl.includes("?") ? "&" : "?"}v=${Date.now()}`,
          thumbUrl: `${fileUrl}${fileUrl.includes("?") ? "&" : "?"}v=${Date.now()}`,
        });
      }
      ...
      /* 6) Crear orden con número secuencial */
      let orderNumber;
      try {
        orderNumber = await generateOrderNumber();
      } catch (err) {
        console.error("Error generando número de orden:", err);
        throw new Error("Error generando número de orden");
      }
      ...
      // 6.1) Enviar email de bienvenida con fotos de evidencia (si hay correo del cliente)
      if (formData.customer.email) {
        try {
          const businessInfo = await getBusinessInfo();
          let persistedOrder = null;
          try {
            persistedOrder = await base44.entities.Order.get(order.id);
          } catch {
            persistedOrder = null;
          }
          const photoUrls = extractEmailPhotoUrls(
            persistedOrder || order || {},
            photos_metadata || []
          );
          const emailData = createWelcomeEmail({
            orderNumber,
            customerName: fullCustomerName || "Cliente",
            deviceInfo: `${formData.device_brand?.name || formData.device_brand || ""} ${formData.device_model?.name || formData.device_model || ""}`.trim(),
          ...
          console.error("[Wizard] Error sending welcome email:", emailErr);
          toast.warning("Orden creada, pero el email no pudo enviarse.");
        }
      }
