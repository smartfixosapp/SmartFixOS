...
const POLL_INTERVAL = 120_000; // 2 min — en iOS Capacitor el polling agresivo calienta el telefono
...
export function useRealtimeNotifications({ enabled = true } = {}) {
  ...
  const checkNotifications = async () => {
    try {
      ...
      // 1. Verificar logins recientes de empleados (DESACTIVADO PARA REDUCIR RUIDO)
      /* 
      ...
      */
      ...
      const isNetworkFail = msg.includes("Load failed") || msg.includes("Failed to fetch") || msg.includes("NetworkError");
      if (isNetworkFail) {
        networkFailCount.current += 1;
        if (networkFailCount.current >= 3) {
          setIsActive(false);
        }
        return;
      }
