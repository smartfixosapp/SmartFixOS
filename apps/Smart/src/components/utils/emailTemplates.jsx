...
export function createWelcomeEmail({ 
  ...
  const photosHTML = photoUrls.length ? `
    <div style="margin: 35px 0;">
      ...
      <div style="margin-top: 16px; background: #F9FAFB; border: 1px solid #E5E7EB; border-radius: 10px; padding: 12px;">
        <p style="margin: 0 0 8px 0; color: #4B5563; font-size: 12px; font-weight: 700;">Enlaces directos de evidencia:</p>
        ${photoUrls.slice(0, 10).map((url, i) => `
          <p style="margin: 4px 0;">
            <a href="${url}" target="_blank" rel="noopener noreferrer" style="color: #2563EB; font-size: 12px; text-decoration: underline;">
              Ver foto ${i + 1}
            </a>
          </p>
        `).join('')}
...
export function createStatusChangeEmail({
  ...
  const statusMessages = {
    ...
    delivered: {
      ...
      message: 'Gracias por confiar en nosotros. Esperamos que disfrutes tu equipo como nuevo.',
      emoji: '🙏',
      showReview: true,
  ...
  const warrantySection = statusInfo.showWarranty && warrantyText ? `
...
/**
 * Email de cotización con el mismo formato bonito
 */
export function createQuoteEmail({
  partName,
  priceOptions = [],
  ...
  const optionsHTML = priceOptions.map((price, index) => `
    <div style="background: white; padding: 28px; margin-bottom: 20px; border-radius: 16px; border: 2px solid #E5E7EB; box-shadow: 0 4px 16px rgba(0,0,0,0.06);">
      <div style="display: flex; align-items: center; margin-bottom: 20px; padding-bottom: 16px; border-bottom: 2px solid #F3F4F6;">
        <div style="background: linear-gradient(135deg, #00A8E8 0%, #10B981 100%); color: white; width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: bold; font-size: 18px; margin-right: 14px; box-shadow: 0 4px 12px rgba(0,168,232,0.3);">
          ${index + 1}
        </div>
        <h3 style="color: #111827; margin: 0; font-size: 20px; font-weight: 700;">Opción ${index + 1}</h3>
      </div>
      
      <div style="space: 0;">
        <div style="display: flex; justify-content: space-between; padding: 14px 0; border-bottom: 1px solid #F3F4F6;">
          <span style="color: #6B7280; font-size: 15px;">Costo de Pieza:</span>
          <span style="color: #111827; font-weight: 700; font-size: 15px;">$${price.partCost.toFixed(2)}</span>
        </div>
        <div style="display: flex; justify-content: space-between; padding: 14px 0; border-bottom: 1px solid #F3F4F6;">
          <span style="color: #6B7280; font-size: 15px;">Mano de Obra:</span>
          <span style="color: #111827; font-weight: 700; font-size: 15px;">$${price.laborCost.toFixed(2)}</span>
        </div>
        <div style="display: flex; justify-content: space-between; padding: 14px 0; border-bottom: 2px solid #E5E7EB; margin-bottom: 14px;">
          <span style="color: #6B7280; font-size: 15px;">Subtotal:</span>
          <span style="color: #111827; font-weight: 800; font-size: 15px;">$${price.subtotal.toFixed(2)}</span>
        </div>
        <div style="display: flex; justify-content: space-between; padding: 14px 0; color: #00A8E8;">
          <span style="font-size: 15px; font-weight: 600;">IVU (11.5%):</span>
          <span style="font-weight: 800; font-size: 15px;">+$${price.ivu.toFixed(2)}</span>
        </div>
      </div>
      
      <div style="background: linear-gradient(135deg, #10B981 0%, #059669 100%); border-radius: 14px; padding: 24px; margin-top: 24px; text-align: center; box-shadow: 0 6px 24px rgba(16,185,129,0.3);">
        <p style="color: rgba(255,255,255,0.95); margin: 0 0 10px 0; font-size: 14px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.8px;">Total a Pagar</p>
        <p style="color: white; margin: 0; font-size: 42px; font-weight: 800; letter-spacing: -1px;">$${price.total.toFixed(2)}</p>
      </div>
    </div>
  `).join('');
  ...
  return {
    ...
    body: `
      ...
      <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Cotización${partName ? ` - ${partName}` : ''}</title>
      </head>
      <body style="margin: 0; padding: 20px; background: #F3F4F6;">
        <div style="${EMAIL_STYLES.container}">
          <!-- Header con gradiente del logo -->
          <div style="${EMAIL_STYLES.header}">
            ...
            <p style="${EMAIL_STYLES.headerSubtitle}">Precio estimado para tu servicio</p>
          </div>
          ...
          <div style="${EMAIL_STYLES.body}">
            ...
            ${partName ? `
              <div style="${EMAIL_STYLES.alert} background: #F0FDF4; border-left-color: #10B981;">
                ...
                <p style="margin: 12px 0 0 0; color: #064E3B; font-size: 16px; line-height: 1.6;">
                  A continuación, las opciones de precio disponibles para este servicio
                </p>
              </div>
            ` : ''}
            ...
            <p style="${EMAIL_STYLES.text}">
              Estas cotizaciones son válidas por <strong style="color: #111827;">7 días</strong>. 
              Para proceder o si tienes alguna pregunta, no dudes en contactarnos.
            </p>
...
export async function getBusinessInfo() {
  try {
    ...
    // Añadir logo URL y textos de garantía
    if (brandingConfigs?.length) {
      const brandingPayload = brandingConfigs[0].payload || {};
      businessData.logo_url = brandingPayload.logo_url || DEFAULT_LOGO_URL;
      businessData.warranty_repairs = brandingPayload.warranty_repairs || "";
      businessData.warranty_sales = brandingPayload.warranty_sales || null;
    } else {
      businessData.logo_url = DEFAULT_LOGO_URL;
    }
