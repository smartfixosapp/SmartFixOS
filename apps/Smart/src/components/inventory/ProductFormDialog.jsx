...
export default function ProductFormDialog({ open, onClose, product = null, onSuccess }) {
  ...
  const handleSubmit = async () => {
    if (!formData.name.trim() || !formData.price || !formData.cost) {
      toast.error("Completa los campos requeridos: Nombre, Precio de Venta, Precio de Compra");
      return;
    }
    ...
    // UI Optimista: actualizar inmediatamente la lista local
    ...
    try {
      ...
      queryClient.setQueryData(['products'], (oldData) => {
        ...
        if (product) {
          // Actualización
          return oldData.map(p => p.id === product.id ? optimisticProduct : p);
        } else {
          // Creación
      ...
      if (product) {
        ...
        queryClient.setQueryData(['products'], (oldData) => {
          return oldData.map(p => 
            p.id === optimisticProduct.id ? { ...created } : p
          );
        });
      }
