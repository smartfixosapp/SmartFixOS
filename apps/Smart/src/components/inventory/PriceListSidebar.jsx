...
export default function PriceListSidebar({ open, onClose, onSelectItem, context = "inventory" }) {
  ...
  return (
    <div className="fixed inset-y-0 right-0 w-96 bg-gradient-to-b from-[#2B2B2B] to-black border-l border-[#FF0000]/30 z-50 flex flex-col shadow-2xl">
      {/* Header */}
        ...
        {loading ? (
          ...
          filteredItems.map((item) => (
            <div
              ...
              className="p-3 bg-gray-900 border border-gray-800 rounded-lg hover:border-[#FF0000]/50 transition-all cursor-pointer group"
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <ChevronRight className="w-5 h-5 text-gray-600 group-hover:text-[#FF0000] transition-colors flex-shrink-0" />
              </div>
            </div>
          ))
        )}
