...
export default function ImportPODialog({ open, onClose, suppliers = [], products = [], workOrders = [], existingPOs = [] }) {
  ...
  const [paymentMethod, setPaymentMethod] = useState("paypal_credit");
  const [settlesOn, setSettlesOn] = useState("");
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        supplier_id: supplierId || "",
        supplier_name: supplier?.name || extracted?.supplier_name || "",
        status: requiresApproval ? "pending" : (paidAtOrder ? "ordered" : "draft"),
