...
export function planFor(productId: unknown): string | null {
  if (productId === SOLO) return "solo";
  if (productId === TEAM) return "team";
  return null;
}

export async function patchTenant(id: string, updates: Record<string, unknown>): Promise<void> {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!jws) return json({ error: "missing signedTransaction" }, 400);

    const tx = await verifyAppleJWS(jws);
    const tenantId = tx.appAccountToken as string | undefined;
    const plan = planFor(tx.productId);
    if (!tenantId) return json({ error: "no appAccountToken in transaction" }, 400);
    if (!plan) return json({ error: "unknown product: " + tx.productId }, 400);
    ...
    return json({ ok: true, plan: active ? plan : "expired" });
  } catch (e) {
