  ...
  async me() {
    if (isDeno) {
      ...
      try {
        ...
        if (!data) {
          ...
          if (createError) {
            // If it's a unique constraint violation on email, try to capture and link existing record
    ...
    try {
      ...
      if (!data) {
        ...
        if (createError) {
          // Final fallback for race conditions or existing record with slightly different case
          if (createError.code === '23505' || createError.message?.includes('users_email_key')) {
            const { data: finalRecord } = await client
              .from(USERS_TABLE_NAME)
              .select("*")
              .ilike("email", user.email)
              .maybeSingle();
