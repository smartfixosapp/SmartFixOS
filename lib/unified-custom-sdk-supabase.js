...
export class UnifiedEntity {
  ...
  async create(data) {
    ...
    // Bridge for RLS: ensure the INSERT request sends the JWT (avoids PostgREST running as anon).
    if (isBrowser && !this.useServiceRole && sessionState?.userId && typeof window !== 'undefined') {
      try {
        const { data: { session } } = await client.auth.getSession();
  ...
  async me() {
    if (isDeno) {
      ...
      try {
        ...
        // If user doesn't exist in users table, try matching by email (case-insensitive)
        if (!data) {
          ...
          if (emailMatch) {
            if (!isProduction) {
              console.log(`🔧 Deno: Found existing user by email ${authUser.email} (case-insensitive), linking auth_id...`);
            }
            ...
            if (!updateError && updated) {
              const mappedUser = this.mapResultFields(updated);
          ...
          if (createError) {
            // Absolute Robustness: If ANY error occurs during insert, try one last time to find/link
            if (!isProduction) console.warn("Insert conflict or error, attempting last-resort link by email...", createError.message);
            
            const { data: lastResortLookup } = await client
              .from(USERS_TABLE_NAME)
              .select("*")
              .ilike("email", authUser.email)
              .maybeSingle();
            ...
            if (lastResortLookup) {
              const { data: finalUpdate } = await client
                .from(USERS_TABLE_NAME)
                .update({ auth_id: authUser.id })
                .eq("id", lastResortLookup.id)
                .select()
                .single();
              if (finalUpdate) {
                const mappedUser = this.mapResultFields(finalUpdate);
                globalClientCache.set(userCacheKey, { data: mappedUser, cachedAt: Date.now() });
                return mappedUser;
              }
            }
            console.error("Critical error in user resolution:", createError);
            return this.mapResultFields(newUser);
          }
          if (!createdUser) return this.mapResultFields(newUser);
          const mappedUser = this.mapResultFields(createdUser);
          // Cache the user data
          globalClientCache.set(userCacheKey, { data: mappedUser, cachedAt: Date.now() });
          return mappedUser;
        }
        ...
        // Cache the user data
        globalClientCache.set(userCacheKey, { data: mappedUser, cachedAt: Date.now() });
        if (!isProduction) console.log("🔧 Deno: Resolution SUCCESS (existing user)");
        return mappedUser;
      } catch (error) {
        console.error("🔴 Deno me() FATAL ERROR:", error);
        // ABSOLUTE FALLBACK to avoid blank screen
        return {
          id: "service-user",
          email: "service@ownmy.app",
          role: "admin",
          full_name: "Service User (Emergency Fallback)"
        };
      }
    }
    ...
    try {
      ...
      if (!data) {
        ...
        if (emailMatch) {
          console.log(`Found existing user by email ${user.email} (case-insensitive), linking auth_id...`);
          const { data: updated, error: updateError } = await client
            .from(USERS_TABLE_NAME)
        ...
        if (createError) {
          // Absolute Robustness: If ANY error occurs during insert, try one last time to find/link
          console.warn("Insert conflict or error, attempting last-resort link by email...", createError.message);
          
          const { data: lastResortRecord } = await client
            .from(USERS_TABLE_NAME)
            .select("*")
            .ilike("email", user.email)
            .maybeSingle();
          ...
          if (lastResortRecord) {
            const { data: finalUpdate } = await client
              .from(USERS_TABLE_NAME)
              .update({ auth_id: user.id })
              .eq("id", lastResortRecord.id)
              .select()
              .single();
            if (finalUpdate) return this.mapResultFields(finalUpdate);
          }
          console.error("Critical error in user resolution:", createError);
          throw createError;
        }
        return createdUser ? this.mapResultFields(createdUser) : this.mapResultFields(newUser);
      }
      ...
      if (!isProduction) console.log("🔧 Browser: Resolution SUCCESS");
      return this.mapResultFields(data);
    } catch (error) {
      console.error("🔴 Browser me() FATAL ERROR:", error);
      
      // If we are in the browser, failing me() often means we need to re-auth
      // BUT we return a dummy object first to prevent the UI from crashing instantly
      const fallbackUser = {
        id: "offline-user",
        email: "offline@local",
        role: "user",
        full_name: "Offline User (Fallback)"
      };
      ...
      if (
        ...
        if (isBrowser) {
          console.warn("🔧 Browser: Auth failure detected, clearing session...");
          getSupabaseClient().then(c => c?.auth.signOut()).catch(() => {});
        }
      }
      
      // Return fallback instead of throwing to keep Dashboard alive
      return fallbackUser;
    }
  }
