  ...
  const integrationsModule = {
    Core: {
      ...
      UploadFile: async ({ file }) => {
        ...
        // Use service role key via direct fetch — bypasses Storage RLS for PIN users
        // (service key is already exposed client-side via VITE_ prefix)
        const base = supabaseUrl.replace(/\/$/, "");
        const key  = supabaseServiceKey;

        // Sanitize filename: Supabase rejects special/non-ASCII chars
        ...
        const res = await fetch(
          `${base}/storage/v1/object/uploads/${storagePath}`,
          {
            method: "POST",
            headers: {
        ...
        const file_url = `${base}/storage/v1/object/public/uploads/${storagePath}`;
        return { file_url };
