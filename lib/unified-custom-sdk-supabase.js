...
// Tables that should NOT be filtered by tenant_id (truly global, no per-tenant rows)
const TENANT_EXEMPT_TABLES = new Set([
  'tenant', 'system_config', 'subscription',
  'device_category', 'part_type', 'accessory_category',
  'work_order_wizard_config',
]);

// Tables with SHARED data: global rows (tenant_id IS NULL) + own rows (tenant_id = current).
// Reads show both; writes always use the current tenant_id; updates/deletes only own rows.
const TENANT_SHARED_TABLES = new Set([
  'brand', 'device_family', 'device_model', 'device_subcategory',
]);
...
// Configuration — todos los secretos salen de .env (ningún fallback hardcoded)
const PROD_API_BASE_URL = "https://smartfixos.onrender.com";
const PROD_FUNCTIONS_BASE_URL = "https://smartfixos.onrender.com";
...
if (!supabaseUrl || !supabaseAnonKey) {
  throw new Error("[unified-custom-sdk-supabase] VITE_SUPABASE_URL y VITE_SUPABASE_ANON_KEY deben estar en .env");
}
...
async function getSupabaseClient() {
  if (isBrowser) {
    ...
    try {
      ...
      if (typeof window !== 'undefined' && !window.supabaseInstance && supabase) {
        window.supabaseInstance = supabase;
      }
    } catch (error) {
      console.warn("Could not import supabase-client.js, checking for existing instance...");
      if (typeof window !== 'undefined' && window.supabaseInstance) {
          supabase = window.supabaseInstance;
      }
    }
...
// App URL (Vercel serverless functions host). Usado para prefijar /api/* cuando
// corremos dentro de Capacitor nativo (iOS/Android), donde "/api/..." resuelve
// al scheme interno del WebView y no llega al servidor.
const PROD_APP_URL = "https://smart-fix-os-smart.vercel.app";
const APP_URL = (getEnvVar("VITE_APP_URL", PROD_APP_URL) || PROD_APP_URL).replace(/\/$/, "");

function resolveBrowserApiUrl(path) {
  const p = String(path || "");
...
async function initializeSupabaseClient(useServiceRole = false, accessToken = null) {
  ...
  // ── Reusar el singleton principal del app si estamos en browser ──────────
  // /lib/supabase-client.js exporta un singleton y lo guarda en
  // window.supabaseInstance. Si lo reutilizamos evitamos el warning
  // "Multiple GoTrueClient instances detected", eliminamos token refresh
  // duplicado, y reducimos el tiempo de inicialización del app.
  // Solo aplica cuando NO es service role (service role necesita su propio
  // cliente con auth deshabilitada).
  if (isBrowser && !actualUseServiceRole && typeof window !== 'undefined' && window.supabaseInstance) {
    globalClientCache.set(cacheKey, window.supabaseInstance);
...
export class UnifiedEntity {
  ...
  async getClient() {
    ...
    if (!this._client) {
      ...
      this._client = typeof window !== 'undefined' && window.supabaseInstance 
        ? window.supabaseInstance 
        : await initializeSupabaseClient(this.useServiceRole, null);
      this._cachedToken = null;
    }
    return this._client;
  }
  ...
  mapResultFields(data) {
    ...
    const mapObject = (obj) => {
      ...
      // Second pass: real DB timestamps (created_at, updated_at) always win over
      // any same-named text columns (e.g. order table has both created_at timestamptz
      // AND created_date text — the text column must not overwrite the real timestamp)
      for (const [srcKey, dstKey] of Object.entries(reverseFieldMappings)) {
        if (srcKey in obj && obj[srcKey] != null) {
          mapped[dstKey] = obj[srcKey];
        }
      }
      return mapped;
    };
  ...
  async list(orderBy = "-created_at", limit = null, skip = null, fields = null) {
    ...
    if (isBrowser && !this.useServiceRole && !TENANT_EXEMPT_TABLES.has(this.tableName)) {
      ...
      if (tenantId) {
        if (TENANT_SHARED_TABLES.has(this.tableName)) {
          // Show global records (tenant_id IS NULL) + own records
  ...
  async create(data) {
    ...
    // TENANT_SHARED_TABLES also get tenant_id injected so new records belong to the tenant
    if (isBrowser && !this.useServiceRole && !TENANT_EXEMPT_TABLES.has(this.tableName)) {
      const tenantId = typeof localStorage !== 'undefined' ? localStorage.getItem('smartfix_tenant_id') : null;
...
console.log("[RLS debug] campaign INSERT policies (with_check):",
  ...
  async update(id, data) {
    ...
    let q = client.from(this.tableName).update(mappedData).eq("id", id);

    // For shared catalog tables, restrict updates to own records only (not globals)
    if (isBrowser && !this.useServiceRole && TENANT_SHARED_TABLES.has(this.tableName)) {
      const tenantId = typeof localStorage !== 'undefined' ? localStorage.getItem('smartfix_tenant_id') : null;
      if (tenantId) q = q.eq('tenant_id', tenantId);
    }

    const { data: result, error } = await q.select().maybeSingle();
  ...
  async delete(id) {
    ...
    let q = client.from(this.tableName).delete().eq("id", id);

    // For shared catalog tables, restrict deletes to own records only (not globals)
    if (isBrowser && !this.useServiceRole && TENANT_SHARED_TABLES.has(this.tableName)) {
      const tenantId = typeof localStorage !== 'undefined' ? localStorage.getItem('smartfix_tenant_id') : null;
      if (tenantId) q = q.eq('tenant_id', tenantId);
    }
  ...
  async me() {
    if (isDeno) {
      ...
      try {
        ...
        if (!isProduction) {
          console.error("Error in Deno me():", error?.message || error);
        }
  ...
  const integrationsModule = {
    Core: {
      ...
      SendEmail: async ({
        ...
        try {
          if (isDeno) {
            // En Deno (Functions server): llamar Resend directamente — no hay URL base para fetch relativo
            const apiKey = Deno.env.get('RESEND_API_KEY');
            const fromEmail = from_email || Deno.env.get('FROM_EMAIL') || 'noreply@smartfixos.com';
            if (!apiKey) throw new Error('RESEND_API_KEY not set in Deno environment');
          ...
          // En browser: usar la ruta Vercel /api/send-raw-email
          // (en Capacitor nativo se prefija con APP_URL — ver resolveBrowserApiUrl)
          const response = await fetch(resolveBrowserApiUrl('/api/send-raw-email'), {
            method: 'POST',
      ...
      UploadFile: async ({ file, category }) => {
        ...
        // Organizar archivos por tenant para aislamiento y visibilidad en admin panel
        // Path: {tenant_id}/{category}/{timestamp}_{filename}
        // Si no hay tenant (ej. super admin) usa solo {category}/{timestamp}_{filename}
        let tenantId = null;
        try {
          tenantId = localStorage.getItem('smartfix_tenant_id');
        } catch { /* non-browser or blocked */ }
        const folder = tenantId
          ? `${tenantId}/${category || 'general'}`
          : (category || 'general');
