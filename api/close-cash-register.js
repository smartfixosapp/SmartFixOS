...
export default async function handler(req, res) {
  ...
  try {
    const { drawerId, denominations = {}, user = {}, summary = {}, tenantId = null } = req.body || {};
    ...
    try {
      ...
      console.warn('close-cash-register movement warning:', movementError.message);
    }

    return res.status(200).json({ success: true, drawer, difference });
  } catch (error) {
    console.error('close-cash-register error:', error.message);
